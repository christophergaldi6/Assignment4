...
void Simulation::runSim(string fileName)
{
  ...
  if (good)  //getting amount of lines in the text file
  {
  ...
  if (good)//initializing all the variables
  {
    ...
    {
      good = false;
      cout << "ERROR: input syntax incorrect. Cannot open more than 5 windows or less than 1." << endl;
    }
  }
...
//decrypting the array into a queue of students
  for (int i = 1; i < input.getLines() - 1 && good; ++i)
  {
    ...
    for (int j = 0; j < numStudents; ++j) //number of students
    {
      ...
      {
        Student s(t, atoi(inputArray[i].c_str()));//creating a new student,"s", with an arrival time,"t"
    ...
    for (int i = 0; i < windowsOpen; ++i)
    {
      ...
      else
      {
        windows[i].idleTime++;
          ...
          //stimulating the arrival of each students
      if (!studentQueue.isEmpty() && studentQueue.peek().arrivalTime <= time)
      {
        if (windows[i].isOpen && !windows[i].isBusy)
        {
          ...
          windows[i].student.waitTime = time - windows[i].student.arrivalTime; // setting wait times for students
...
{
  //calculating stats for students wait times
